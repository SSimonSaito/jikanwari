...
TEACHERS = {
    ...
    '田代先生': {'教科': ['生物基礎', '生物', '地学基礎', '地学', '科学と人間生活', '理科（選択）'], '上限': 15},
    '小東先生': {'教科': ['英語コミュニケーションⅠ', '英語コミュニケーションⅡ', '英語コミュニケーションⅢ',
                       '論理・表現Ⅰ', '論理・表現Ⅱ', '論理・表現Ⅲ'], '上限': 15},
    '桜木先生': {'教科': ['世界史A', '世界史B', '日本史A', '日本史B', '地理A', '地理B', '現代社会', '倫理', '政治・経済', '公共', '倫理/政経'], '上限': 15},
    '齋須先生': {'教科': ['保健', '体育'], '上限': 15},
    '佐藤先生': {'教科': ['音楽Ⅰ', '音楽Ⅱ', '美術Ⅰ', '美術Ⅱ', '書道Ⅰ', '書道Ⅱ', '芸術科目'], '上限': 15},
        ...
        for cls in CLASSES:
            ...
            for subject, count in subjects.items():
                for _ in range(count):
                    assigned = False
                    attempts = 0
                    while not assigned and attempts < len(all_slots):
                        day, period = all_slots[slot_index % len(all_slots)]
                        slot_index += 1
                        attempts += 1
                        for t, data in TEACHERS.items():
                            if (subject in data['教科'] and
                                teacher_assignments[t] < data['上限'] and
                                t not in slot_usage[(day, period)]):
                                timetable.append({
                                    '学年': grade, '組': cls, '曜日': day, '時限': period,
                                    '教科': subject, '教師': t
                                })
                                teacher_assignments[t] += 1
                                slot_usage[(day, period)].add(t)
                                assigned = True
                                break
                    if not assigned:
                        timetable.append({
                            '学年': grade, '組': cls, '曜日': '未定', '時限': '未定',
                            '教科': subject, '教師': '未割当'
                        })
